      ...
      <div style={{display:'grid',gridTemplateColumns:'1.5fr 1fr',gap:40,alignItems:'flex-start'}}>
        ...
        <aside style={{position:'sticky',top:90,padding:24,background:'var(--bg-soft)',borderRadius:14,border:'1px solid var(--line)'}}>
          ...
          <div style={{display:'flex',gap:8,marginBottom:18}}>
            <input placeholder="Promo code" className="" style={{flex:1,padding:'10px 12px',border:'1px solid var(--line)',borderRadius:8,fontSize:13,fontFamily:'var(--body)',background:'var(--bg)'}}/>
...
function CheckoutPage({ cart, go, address, setAddress }) {
...
// ─── PAYMENT ───
function PaymentPage({ cart, go, address, onPlaceOrder }) {
  ...
  function pay() {
    setProcessing(true);
    setTimeout(() => { onPlaceOrder(); go('confirmation'); }, 1600);
  }
...
// ─── CONFIRMATION ───
function ConfirmationPage({ cart, address, go }) {
  const sub = cart.reduce((s,it)=>s+it.price*it.qty,0);
  const ship = sub > 999 ? 0 : 99;
  const tax = Math.round(sub * 0.05);
  const total = sub + ship + tax;
  const orderId = 'SAI-2087-' + Math.floor(4500 + Math.random()*500);
              ...
              <div style={{display:'flex',flexDirection:'column',gap:12}}>
                {ORDERS.map(o => (
                  <div key={o.id} className="card" style={{padding:20,display:'grid',gridTemplateColumns:'auto 1fr auto auto',gap:20,alignItems:'center'}}>
                    <div style={{width:60,height:60,borderRadius:8,overflow:'hidden'}}><ProdImg kind={['lota','bottle','thali','diya'][Math.floor(Math.random()*4)]} material={['brass','copper'][Math.floor(Math.random()*2)]} size={40}/></div>
                    <div>
...
Object.assign(window, { CartPage, CheckoutPage, PaymentPage, ConfirmationPage, TrackingPage, LoginPage, AccountPage });
