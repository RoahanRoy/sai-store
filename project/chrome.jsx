// Header, Footer, ProductCard, CartDrawer, Search Overlay, FestiveStrip
// Shared across all screens

function FestiveStrip({ show }) {
  if (!show) return null;
  return (
...
function Header({ onSearch, onCart, onMenu, cartCount = 0, route, go, mobile }) {
  if (mobile) {
    return (
...
function ProductCard({ p, onClick, variant = 'default', onAdd }) {
...
function SearchOverlay({ open, onClose, go }) {
  ...
  return (
    <div onClick={onClose} style={{position:'absolute',inset:0,background:'rgba(20,15,10,.55)',zIndex:90,display:'flex',justifyContent:'center',alignItems:'flex-start',paddingTop:80,backdropFilter:'blur(4px)'}}>
      <div onClick={e=>e.stopPropagation()} className="fadeup" style={{width:'min(640px, 92%)',background:'var(--bg)',borderRadius:14,boxShadow:'var(--shadow-lg)',overflow:'hidden'}}>
        ...
        <div style={{padding:20,maxHeight:480,overflowY:'auto'}}>
          ...
          {q && (
            <>
              <div style={{fontSize:11,letterSpacing:'.2em',textTransform:'uppercase',color:'var(--ink-mute)',marginBottom:10}}>{results.length} result{results.length!==1?'s':''} for "{q}"</div>
              {results.map(p => (
                <div key={p.id} onClick={()=>{go('product',p.id);onClose();}} style={{display:'flex',gap:12,padding:8,borderRadius:8,cursor:'pointer',alignItems:'center'}} onMouseEnter={e=>e.currentTarget.style.background='var(--bg-soft)'} onMouseLeave={e=>e.currentTarget.style.background='transparent'}>
                  <div style={{width:48,height:48,flexShrink:0}}><ProdImg kind={p.kind} material={p.material} size={36}/></div>
                  <div style={{flex:1}}>
              ...
              {results.length === 0 && <div style={{padding:'30px 0',textAlign:'center',color:'var(--ink-mute)',fontSize:13}}>No matches. Try "lota", "thali" or "copper".</div>}
            </>
          )}
        </div>
      </div>
...
function CartDrawer({ open, onClose, cart, setCart, go }) {
  ...
  return (
    <div onClick={onClose} style={{position:'absolute',inset:0,background:'rgba(20,15,10,.45)',zIndex:90,display:'flex',justifyContent:'flex-end',backdropFilter:'blur(2px)'}}>
...
// ─── Mobile menu ───
function MobileMenu({ open, onClose, go, route }) {
  if (!open) return null;
  const links = [
    { id:'home', label:'Home' },
    { id:'shop', label:'Shop All' },
  ...
  return (
    <div onClick={onClose} style={{position:'absolute',inset:0,background:'rgba(20,15,10,.5)',zIndex:90}}>
...
Object.assign(window, { FestiveStrip, TopStrip, Header, Footer, ProductCard, SearchOverlay, CartDrawer, MobileMenu });
